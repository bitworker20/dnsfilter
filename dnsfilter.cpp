...
namespace DNS
{
			...
			Name &name)
		{
			...
			while (true)
			{
				...
				if ((len & 0xc0u) == 0xc0u)
				{
					...
					if (last_offset && off == last_offset)
					{ // reference loop
						return false;
					}
	...
	bool parse(const uint8_t *origin, size_t size, Packet &packet)
	{
		...
		if (!read16(data, end, packet.ID) || !read16(data, end, FLAGS) || !read16(data, end, QDCOUNT) || !read16(data, end, ANCOUNT) || !read16(data, end, NSCOUNT) || !read16(data, end, ARCOUNT))
		{
			return false;
		}
...
private:
	struct Address
	{
		...
		Address(const sockaddr &address, socklen_t addrlen)
			: address(address), addrlen(addrlen)
		{
		}
...
{
	return filter_addresses(packet.answer) || filter_addresses(packet.authority) || filter_addresses(packet.additional);
}
...
void DnsServer::forward_request(
...
void DnsServer::send_reply(const DNS::Packet &packet, const Address &client_addr)
{
	...
	for (auto &i : packet.answer)
	{
		...
		{
			std::cout << TEXT::Green << "R: " << DNS::build_name(i.r_name, 0) << "[" << i.r_type << "]"
					  << " -> " << DNS::get_address(i) << TEXT::Reset << '\n';
		}
	}
	...
	for (auto &i : packet.authority)
	{
		if (i.r_type == DNS::Type::A || i.r_type == DNS::Type::AAAA)
		{
			std::cout << TEXT::Cyan << "R: " << DNS::build_name(i.r_name, 0) << "[" << i.r_type << "]"
					  << " -> " << DNS::get_address(i) << TEXT::Reset << '\n';
		}
	}
...
void DnsServer::get_bind_addr(struct in_addr &sin_addr)
{
